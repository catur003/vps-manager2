...
router.post('/', (req, res) => {
  ...
  const auditId = audit.recordStart({ action: ACTION, ip: req.ip, params: { user, schedule, command } });
  const result = cron.add(user, schedule, command);
...
router.put('/:index', (req, res) => {
  ...
  const startedAt = Date.now();
  const auditId = audit.recordStart({ action: ACTION, ip: req.ip, params: { user, index, schedule, command } });
  const result = cron.update(user, index, schedule, command);
  audit.recordEnd(auditId, { success: result.ok, message: result.errorMessage || 'OK', durationMs: Date.now() - startedAt });
