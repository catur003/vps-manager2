...
# FIXED: self-heal path di database.js setupRootDatabase() kalau dipanggil
# sebagai deploy_user (bukan root - mis. dari bin/vps-bootstrap.js step 1/6)
# butuh `sudo mysql -e ...` (socket auth sebagai root MariaDB) dan
# `sudo systemctl enable --now mariadb`, tapi dua-duanya tidak pernah ada di
# rule manapun - jadi path itu selalu mentok "password is required".
# Konsekuensi keamanan disetujui eksplisit oleh user: `sudo mysql` tanpa
# argumen tetap = akses root PENUH ke database (sama levelnya kayak entry
# bare lain di file ini: certbot/openssl/tee/tail) - diterima karena
# mainMenu.js "Buat User Admin DB" juga butuh `sudo mysql --socket=...`.
BIN_MYSQL="$(resolve_bin mysql)"
BIN_PSQL="$(resolve_bin psql)"
...
${API_USER} ALL=(root) NOPASSWD: ${NGINX_BINARY} -t, ${NGINX_BINARY} -s reload, /bin/systemctl restart nginx, /bin/systemctl reload nginx, /bin/systemctl enable --now mariadb, /usr/bin/certbot, ${BIN_MYSQL}, ${BIN_OPENSSL}, /usr/bin/ss -tlnp, /bin/ss -tlnp, /usr/sbin/ufw status, ${BIN_FIREWALL_CMD} --state, ${BIN_IPTABLES} -L INPUT -n, /usr/bin/lsof, ${BIN_LS}, ${BIN_CAT}, ${BIN_CP}, ${BIN_LN}, ${BIN_MKDIR}, ${BIN_CHMOD}, ${BIN_CHOWN}, ${BIN_RM}, ${BIN_TEST}, ${BIN_GREP}, ${BIN_FAIL2BAN} status*, ${BIN_PM2} list, ${BIN_APT_GET} update, ${BIN_APT_GET} install -y python3-certbot-dns-cloudflare, ${BIN_APT_GET} install -y mysql-server, ${BIN_APT_GET} install -y redis-server, ${BIN_APT_GET} install -y certbot, ${BIN_APT_GET} install -y nginx, ${BIN_APT_GET} install -y ufw, ${BIN_APT_GET} install -y fail2ban, ${BIN_APT_GET} install -y git, ${BIN_APT_GET} install -y unzip, ${BIN_APT_GET} install -y htop, ${BIN_APT_GET} install -y ffmpeg, ${BIN_APT_GET} install -y docker.io, ${BIN_APT_GET} install -y python3-certbot-nginx, ${BIN_APT_GET} install -y postgresql, ${BIN_APT_GET} install -y mariadb-server, ${BIN_APT_GET} install -y docker-ce, ${BIN_APT_GET} install -y memcached, ${BIN_APT_GET} install -y build-essential, ${BIN_APT_GET} install -y python3-pip, ${BIN_APT_GET} install -y supervisor, ${BIN_APT_GET} install -y imagemagick, ${BIN_APT_GET} install -y zip, ${BIN_APT_GET} install -y curl, ${BIN_APT_GET} install -y wget, ${BIN_APT_GET} install -y rsync, ${BIN_APT_GET} install -y jq, ${BIN_APT_GET} install -y tmux, ${BIN_APT_GET} install -y vim, ${BIN_TEE}, ${BIN_STAT}, ${BIN_MV}, ${BIN_TAIL}, ${BIN_DOCKER} ps, ${BIN_DOCKER} ps -a, ${BIN_DOCKER} ps --format*, ${BIN_DOCKER} ps -a --format*, ${BIN_DOCKER} start *, ${BIN_DOCKER} stop *, ${BIN_DOCKER} restart *, ${BIN_DOCKER} logs*, ${BIN_DOCKER} inspect*, ${BIN_DOCKER} stats --no-stream*, ${BIN_DOCKER} run *, ${BIN_DOCKER} rm -f *, ${BIN_DOCKER} compose build, ${BIN_DOCKER} compose up -d, ${BIN_DOCKER} compose down, ${BIN_DOCKER} compose restart, ${BIN_DOCKER} compose logs*, ${BIN_DOCKER} compose exec -T app php artisan migrate --force, ${BIN_DOCKER} system df*, ${BIN_DOCKER} image prune -af, ${BIN_DOCKER} builder prune -af, ${BIN_APT_GET} clean, /usr/bin/journalctl --disk-usage, /usr/bin/journalctl --vacuum-time=7d, ${BIN_DOCKER} exec -it * sh, ${BIN_DOCKER} exec -it * bash, ${BIN_DOCKER} exec * which bash, ${BIN_APT_GET} remove -y nginx, ${BIN_APT_GET} remove -y certbot, ${BIN_APT_GET} remove -y python3-certbot-nginx, ${BIN_APT_GET} remove -y mysql-server, ${BIN_APT_GET} remove -y postgresql, ${BIN_APT_GET} remove -y redis-server, ${BIN_APT_GET} remove -y memcached, ${BIN_APT_GET} remove -y docker.io, ${BIN_APT_GET} remove -y build-essential, ${BIN_APT_GET} remove -y python3-pip, ${BIN_APT_GET} remove -y supervisor, ${BIN_APT_GET} remove -y ffmpeg, ${BIN_APT_GET} remove -y imagemagick, ${BIN_APT_GET} remove -y ufw, ${BIN_APT_GET} remove -y fail2ban, ${BIN_APT_GET} remove -y git, ${BIN_APT_GET} remove -y unzip, ${BIN_APT_GET} remove -y zip, ${BIN_APT_GET} remove -y htop, ${BIN_APT_GET} remove -y curl, ${BIN_APT_GET} remove -y wget, ${BIN_APT_GET} remove -y rsync, ${BIN_APT_GET} remove -y jq, ${BIN_APT_GET} remove -y tmux, ${BIN_APT_GET} remove -y vim, ${BIN_APT_GET} remove -y mariadb-server, ${BIN_APT_GET} remove -y docker-ce

${API_USER} ALL=(root) NOPASSWD: /bin/systemctl enable --now postgresql
${API_USER} ALL=(root) NOPASSWD: /bin/systemctl enable --now mysql
${API_USER} ALL=(postgres) NOPASSWD: ${BIN_PSQL}
...
# FIXED: sebelumnya `chmod +x scripts/setup-sudoers.sh` pakai path RELATIF -
# bergantung cwd pemanggil. Kalau script ini dipanggil dari cwd selain root
# repo (mis. `sudo bash /path/vps-manager2/setup-otomatis.sh` dari /root),
# chmod gagal dan karena `set -euo pipefail` script exit non-zero - padahal
# rule sudoers BARU SAJA berhasil ditulis - dan install.sh (juga set -e)
# ikut abort sebelum sempat lanjut ke TLS/bootstrap. Sekarang di-anchor ke
# lokasi script ini sendiri dan dibuat non-fatal (script ini selalu dipanggil
# lewat `bash <path>`, bit executable cuma nice-to-have).
chmod +x "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$(basename "${BASH_SOURCE[0]}")" 2>/dev/null || true
