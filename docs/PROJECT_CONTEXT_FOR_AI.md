...
- **Auth**: 1 API key global (256-bit random, disimpan sebagai scrypt hash di `data/config.json`, plaintext-nya cuma ditampilkan sekali pas generate). Dikirim via `Authorization: Bearer <key>` (REST) atau query `?key=` (WebSocket, karena browser WS handshake gak bisa nyetel header custom).
